...
def load_and_process_data():
    ...
    # Carregar dados da frota usando read_excel
    try:
        df_frota = pd.read_excel(
            "E_Frota_por_UF_Municipio_POTENCIA_Dezembro_2024.xlsx",
            skiprows=3,
            header=None
        )
    ...
    # Tratar datas e horas e converter lat/lon para numérico
...
if df is not None:
    ...
    page = st.sidebar.radio("Selecione uma página", ["Visão Geral", "Análise por Município", "Previsão de Risco"])
    ...
    if page == "Visão Geral":
        ...
        # MELHORIA: Análise de dados geográficos com transparência
        st.subheader("🗺️ Análise Geográfica dos Acidentes")
        
        map_data = df.dropna(subset=['latitude', 'longitude'])
        total_com_geo = len(map_data)
        percentual_geo = (total_com_geo / total_acidentes) * 100
        ...
        if map_type == "Pontos de Acidente":
            fig_map = px.scatter_mapbox(
                ...
                hover_data={"causa_acidente": True, "data_inversa": "|%d de %B de %Y"},
                size_max=15,
                zoom=3.5,
                center=dict(lat=-14.2350, lon=-51.9253),
                height=600,
                mapbox_style="carto-positron",
                title="Distribuição de Pontos de Acidente no Brasil"
            )
            # Diminui o tamanho e opacidade dos marcadores para melhor visualização
            ...
            fig_map = px.density_mapbox(
                ...
                title="Concentração de Acidentes no Brasil"
            )
    ...
    # ----- Página: Análise por Município -----
        ...
        map_data_municipio = df_municipio.dropna(subset=['latitude', 'longitude'])

        if not map_data_municipio.empty:
            map_center = dict(lat=map_data_municipio['latitude'].mean(), lon=map_data_municipio['longitude'].mean())
            
            fig_map_municipio = px.scatter_mapbox(
                map_data_municipio,
                lat="latitude",
                lon="longitude",
                hover_name="causa_acidente",
