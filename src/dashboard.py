...
try:
    with config_file.open('r') as file:
        ...
        # Validação robusta da estrutura do config.yaml
...
if st.session_state.get("authentication_status"):
    ...
    def load_and_process_data():
        ...
        try:
            # Carrega os dados de acidentes da URL
            df_acidentes = pd.read_csv(URL_ACIDENTES, sep=';', encoding='latin1', on_bad_lines='skip')
        except Exception as e:
            st.error(f"Não foi possível carregar os dados de acidentes da URL. Erro: {e}")
            return None

        try:
            # Carrega os dados da frota da URL
            df_frota = pd.read_excel(URL_FROTA, skiprows=3, header=None)
            df_frota.columns = ['uf', 'municipio', 'total_veiculos', 'potencia']
        except Exception as e:
            st.error(f"Não foi possível carregar os dados da frota da URL. Verifique se o arquivo .xlsx está no GitHub. Erro: {e}")
            return None
    ...
    if df is not None:
        ...
        if page == "Visão Geral":
            ...
            map_type = st.selectbox(
                "Selecione o tipo de visualização do mapa:",
                ("Pontos de Acidente", "Mapa de Calor (Densidade)")
            )

            if map_type == "Pontos de Acidente":
                fig_map = px.scatter_mapbox(
                    df, lat="latitude", lon="longitude", color="classificacao_acidente",
                    hover_name="municipio", hover_data={"causa_acidente": True, "data_inversa": "|%d de %B de %Y"},
                    size_max=15, zoom=3.5, center=dict(lat=-14.2350, lon=-51.9253),
                    height=600, mapbox_style="carto-positron", title="Distribuição de Pontos de Acidente no Brasil"
                )
                fig_map.update_traces(marker=dict(size=5, opacity=0.7))
            else:
                fig_map = px.density_mapbox(
                    df, lat="latitude", lon="longitude", radius=10,
                    center=dict(lat=-14.2350, lon=-51.9253), zoom=3.5,
                    height=600, mapbox_style="carto-positron",
                    color_continuous_scale="YlOrRd", title="Concentração de Acidentes no Brasil"
                )
            ...
            with col1:
                ...
                fig_weekday = px.bar(df_weekday, x=df_weekday.index, y=df_weekday.values,
                                     labels={'x': 'Dia da Semana', 'y': 'Número de Acidentes'},
                                     color_discrete_sequence=["#33C4FF"])
                st.plotly_chart(fig_weekday, use_container_width=True)

            with col2:
                st.subheader("☀️ Acidentes por Período do Dia")
                df_periodo = df['periodo_dia'].value_counts()
                fig_periodo = px.pie(df_periodo, names=df_periodo.index, values=df_periodo.values,
                                     title="Distribuição Dia vs. Noite",
                                     color_discrete_sequence=["#FFC300", "#36454F"])
            ...
            if not df_municipio.empty:
                ...
                fig_map_municipio = px.scatter_mapbox(
                    df_municipio,
                    lat="latitude", lon="longitude", hover_name="causa_acidente",
                    hover_data={"data_inversa": "|%d de %B de %Y"},
                    color="classificacao_acidente", zoom=10, height=500,
                    center=map_center, mapbox_style="carto-positron",
                    title=f"Pontos de Acidente em {municipio}"
                )
            ...
            fig_time = px.line(df_time, x='data_inversa', y='contagem',
                               labels={'data_inversa': 'Data', 'contagem': 'Número de Acidentes'},
            ...
            with col1:
                ...
                fig_causas = px.bar(df_causas, y=df_causas.index, x=df_causas.values, orientation='h',
            ...
            with col2:
                ...
                fig_pista = px.bar(df_pista, x=df_pista.index, y=df_pista.values,
                                   labels={'x': 'Tipo de Pista', 'y': 'Número de Acidentes'},
            ...
            if st.button("Calcular Risco", use_container_width=True, type="primary"):
                input_data = pd.DataFrame([[hora, causa, tipo, clima]],
                                          columns=['hora', 'causa_acidente', 'tipo_acidente', 'condicao_metereologica'])
